...
export default function OverviewStep({ data, updateData }: Props) {
  ...
  const [selectedCategory, setSelectedCategory] = useState(data.category.split(' > ')[0] || '');
  ...
  const removeTag = (tag: string) => {
    updateData('searchTags', data.searchTags.filter(t => t !== tag));
  };
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="relative">
          <div className="absolute left-3 top-3 text-muted-foreground">You will get</div>
          <input
            type="text"
            value={data.title}
            onChange={(e) => updateData('title', e.target.value)}
            placeholder="Tell the client what you will deliver and how it benefits them."
            className="w-full pl-24 pr-3 py-3 border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-ring bg-background"
          />
        </div>
          ...
          <div className="bg-muted p-4 rounded-lg">
            ...
            <p className="text-foreground">
              {data.category.split(' > ').map((part, index) => (
                <span key={index}>
                  {index > 0 && ' › '}
                  <span className={index === 0 ? 'font-semibold' : ''}>{part}</span>
                </span>
              ))}
            </p>
          </div>
