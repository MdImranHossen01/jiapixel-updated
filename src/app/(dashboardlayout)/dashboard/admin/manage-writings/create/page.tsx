...
export default function CreateWritingPage() {
    ...
    const [selectedWritings, setSelectedWritings] = useState<string[]>([]);
    ...
    const toggleWriting = (writingId: string) => {
        if (selectedWritings.includes(writingId)) {
            setSelectedWritings(selectedWritings.filter(id => id !== writingId));
        } else {
            if (selectedWritings.length >= 4) {
                toast.error("You can select up to 4 writings only");
                return;
            }
            setSelectedWritings([...selectedWritings, writingId]);
        }
    };
    ...
    const filteredWritings = writings.filter(writing =>
        writing.title.toLowerCase().includes(writingSearchQuery.toLowerCase())
    );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const writingData = {
                ...
                relatedWritings: selectedWritings
            };
            ...
            if (response.ok) {
                ...
                toast.error(error.error || 'Failed to create writing');
            }
        } catch (error) {
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold text-foreground">Create New Writing</h1>
                    <p className="text-muted-foreground mt-2">
                        Add a new writing piece to your collection
                    </p>
                </div>
                <button
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 gap-6">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-card rounded-lg shadow p-6 border border-border">
                            ...
                            <div className="space-y-4">
                                {loadingWritings ? (
                                    <div className="text-sm text-muted-foreground">Loading writings...</div>
                                ) : (
                                    <>
                                        {/* Search Input */}
                                        <div>
                                            ...
                                            {filteredWritings.length > 0 ? (
                                                <div className="space-y-2">
                                                    {filteredWritings.map((writing) => (
                                                        <label key={writing._id} className="flex items-center space-x-3 cursor-pointer p-2 hover:bg-accent rounded text-foreground">
                                                            <input
                                                                type="checkbox"
                                                                checked={selectedWritings.includes(writing._id)}
                                                                onChange={() => toggleWriting(writing._id)}
                                                                className="h-4 w-4 text-primary border bg-background rounded focus:ring-primary"
                                                            />
                                                            <span className="text-sm">{writing.title}</span>
                                                        </label>
                                                    ))}
                                                </div>
                                            ) : (
                ...
                <div className="flex justify-end space-x-4 pt-6 border-t border-border">
                    ...
                    >
                        {submitting ? 'Creating...' : 'Create Writing'}
                    </button>
                </div>
            </form>
        </div>
