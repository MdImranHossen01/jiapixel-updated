...
const ActionsCell = ({ newsletter }: { newsletter: Newsletter }) => {
    const router = useRouter();
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem
                    onClick={() => navigator.clipboard.writeText(newsletter._id)}
                >
                    Copy ID
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem asChild>
                    <Link href={`/newsletters/${newsletter.slug}`} target="_blank">View Public</Link>
                </DropdownMenuItem>
                <DropdownMenuItem asChild>
                    <Link href={`/dashboard/admin/manage-newsletters/edit/${newsletter.slug}`}>Edit</Link>
                </DropdownMenuItem>
                <DropdownMenuItem
...
export const columns: ColumnDef<Newsletter>[] = [
                ...
                <TooltipProvider>
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <Link href={`/dashboard/admin/manage-newsletters/edit/${slug}`} className="font-medium hover:underline text-primary">
                                {truncated}
                            </Link>
                        </TooltipTrigger>
                        <TooltipContent>
    ...
    },
    {
        id: "actions",
        cell: ({ row }) => {
            return <ActionsCell newsletter={row.original} />
        },
    },
]
