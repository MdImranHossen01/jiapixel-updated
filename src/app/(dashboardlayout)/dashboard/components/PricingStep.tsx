...
export default function PricingStep({ data, updateData }: Props) {
  ...
  const relevantFeatures =
    serviceFeatures[selectedCategory as keyof typeof serviceFeatures] || [];
          ...
          {selectedCategory ? (
            <div className="space-y-2">
              {relevantFeatures.map((feature) => (
                <label
                  key={feature}
                  className="flex items-center space-x-2 p-2 hover:bg-accent/30 rounded transition-colors"
                >
                  <input
                    type="checkbox"
