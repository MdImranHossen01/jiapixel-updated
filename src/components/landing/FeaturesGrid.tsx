...
export function FeaturesGrid({ features }: FeaturesGridProps) {
    ...
    const showMore = () => {
        setVisibleCount((prev) => Math.min(prev + 8, features.length));
    };
            ...
            {hasMore && (
                <div className="flex justify-center pt-8">
                    <Button
                        ...
                        size="lg"
                        variant="outline"
                        className="rounded-full px-8 h-14 text-lg font-bold border border-border hover:border-primary hover:bg-primary/5 transition-all group shadow-xl shadow-primary/5"
                    >
                        <span className="text-primary">See more features</span>
